...
fun AppBar(){
    ...
    {
        Text(text = stringResource(R.string.whatsapp_title),
       ...
       androidx.compose.material3.Icon(painter = painterResource(id = R.drawable.tabler_camera),
           contentDescription ="WhatsApp Camera", tint = if (isSystemInDarkTheme())
           { Color.White}else Color.Black
       )

        Spacer(modifier = Modifier.size(24.dp))
        androidx.compose.material3.Icon(painter = painterResource(id = R.drawable.qr_code_scanner_24),
            contentDescription = "QR Scanner" ,  tint = if (isSystemInDarkTheme())
            { Color.White}else Color.Black)

        Spacer(modifier = Modifier.size(24.dp))
        androidx.compose.material3.Icon(painter = painterResource(id = R.drawable.baseline_more_vert_24),
            contentDescription = "menu", tint = if (isSystemInDarkTheme())
            { Color.White} else Color.Black)
    }

    OutlinedTextField(value = inputText, onValueChange = {inputText = it}, modifier = Modifier
        .fillMaxWidth()
        .padding(horizontal = 14.dp)
  , shape = RoundedCornerShape(30.dp) ,
        leadingIcon ={ androidx.compose.material3.
        Icon(painter = painterResource(id = R.drawable.logos_meta_icon),
            contentDescription = "META LOGO")},
        placeholder = { Text(text = "Ask Meta Ai or Search" , fontSize = 20.sp ,
            fontWeight = FontWeight.Normal , color = if (isSystemInDarkTheme()){
                ThinGray} else ThinBlack
        )},
        colors = TextFieldDefaults.outlinedTextFieldColors(
            containerColor = if (isSystemInDarkTheme()){
                BlackGray} else ThinGray,
            focusedBorderColor = if (isSystemInDarkTheme()){
               BlackGray} else ThinGray,
            unfocusedBorderColor = if (isSystemInDarkTheme()){
                BlackGray} else ThinBlack
